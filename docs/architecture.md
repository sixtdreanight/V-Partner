...
基于 shadcn/ui (Radix UI 原语) + Tailwind CSS 4 构建，使用 lucide-react 图标库。所有组件支持暗色模式。

## 设计令牌系统

所有 UI 组件使用 CSS 自定义属性（`--vp-*` 前缀 + shadcn `--background`/`--foreground`/`--primary` 等标准令牌），定义在 `src/renderer/styles/globals.css`：
